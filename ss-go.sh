...
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/530823374/doubi/master/service/ss_go_centos" -O /etc/init.d/ss-go; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/530823374/doubi/master/service/ss_go_debian" -O /etc/init.d/ss-go; then
			echo -e "${Error} Shadowsocks 服务管理脚本下载失败 !"
			rm -rf "${FOLDER}"
			exit 1
		fi
